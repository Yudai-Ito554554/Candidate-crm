...
最終更新: 2026-08-20（Asia/Tokyo、S3-3・S3-7の実機UAT完了とB-6の実装を反映）
基準: `main` `099d896`
...
3. **S3-3は2026-08-20に完了済み**: Fable 5が2026-08-19に検証方式を再決定した（案D＝UAT要件の再定義）。「web buildをブラウザで開きURLバーから直打ち」は`persistSession: false`とOS資格情報ストア前提のセッション復元により**このコードベースでは成立しない**ため撤回。合格条件は(1)自動テスト7ルート／(2)pgTAP 007／(3)Tauriアプリ内UATの3点で、3点とも達成済み。テスト専用deep-linkは引き続き実装しない。案A（ログイン後の要求ルート復帰）はS3-3の合格条件ではない独立改善としてB-6で実装済み（`src/features/auth/login-redirect.ts`）。詳細は`docs/fable5-decision-s3-3-2026-08-19.md`。
4. **S3-7は2026-08-20に完了済み**（admin実アカウントで企業・求人のアーカイブ／復元を確認。候補者は2026-08-11に確認済み）。
...
- **S3-3の実機検証方法は決着済み（2026-08-20時点で解消）**: Tauriアプリにアドレスバーが無く「URLを直接入力する」再現手段が無い問題は、Fable 5がUAT要件を再定義して解決した（アプリ内で到達可能な全経路からの到達試行へ置き換え、合格の観測は閲覧専用パネルの表示または導線の不在）。自動テストは編集7ルート、pgTAPはviewerの書き込み拒否を固定しており、実機UATも2026-08-20に完了している。
