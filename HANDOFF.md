...
| S3-3  | ☐     | viewerがURL直接入力で編集画面を開けないことの**実アカウントでの直接URL確認**（自動テストは済み）。確認方法はFable 5の判断で決着済み（stagingへ向けたweb buildをブラウザで開く）。**次の作業項目。Supabaseダッシュボードのログイン情報の確認から着手する**（staging接続情報とviewerテストユーザーのパスワードが必要） |
