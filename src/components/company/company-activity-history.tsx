import { BriefcaseBusiness, Clock3, Sparkles, UserRound } from "lucide-react";
...
import type { CandidateRow, JobRow } from "@/types/database";

interface CompanyActivityHistoryProps {
  companyId: string;
...
export function CompanyActivityHistory({
  companyId,
  candidates,
  jobs,
}: CompanyActivityHistoryProps) {
  ...
  return (
    <SectionCard
      description="この企業に紐づく最新30件を新しい順に表示"
      title="企業の活動履歴"
    >
      {activitiesQuery.isPending || profilesQuery.isPending ? (
        ...
        <ol className="divide-y divide-slate-100">
          {activities.map((activity) => {
            const candidate = activity.candidate_id
              ...
              : undefined;
            const job = activity.job_id
              ? jobMap.get(activity.job_id)
              : undefined;
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
                    ...
                    <span className="inline-flex items-center gap-1">
                      ...
                      担当：{owner?.display_name ?? owner?.email ?? "未設定"}
                    </span>
                  </div>
                </div>
              </li>
        ...
        <EmptyState message="この企業に関連する活動はありません" />
      )}
    </SectionCard>
  );
}
