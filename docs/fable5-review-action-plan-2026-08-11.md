...
残Low（独立migrationとしてレビュー用ブランチで対応済み）:
...
状態: Fable 5承認済み。ローカル実装・検証完了、レビュー用ブランチへ公開中。全体検索テストは10回連続成功。全体テストは69ファイル・365件成功し、HydrateFallback警告は0件。`vite.config.ts`には、将来CI時間が問題化してもworker競合を戻さず、重いjsdom suiteを分離する方針を記録した（`7892d64`）。
